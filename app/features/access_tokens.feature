...
Feature: Access tokens endpoint
    ...
    Scenario: Get access tokens success
        ...
        And the response property "data.expiresIn" matches regular expression "\d\d\d\d\d"
        And the response property "data.refreshExpiresIn" matches regular expression "\d\d\d\d\d\d\d\d"
